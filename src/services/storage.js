...
async function saveGame(scopeInput, game) {
  ...
  await withTransaction(async (client) => {
    await client.query(
      `
        INSERT INTO games (
          chat_id, scope_key, platform, scope_type, phase, num_players, setup_step, setup_substep, setup_buffer, history, hidden_memory, current_turn, updated_at
        )
...
async function resetGame(scopeInput) {
  ...
  await withTransaction(async (client) => {
    ...
    await client.query(
      `
        INSERT INTO games (
          chat_id, scope_key, platform, scope_type, phase, num_players, setup_step, setup_substep, setup_buffer, history, hidden_memory, current_turn
        )
        VALUES ($1, $2, $3, $4, 'setup', 0, 0, 'num_players', '{}'::jsonb, '[]'::jsonb, '[]'::jsonb, 0)
      `,
