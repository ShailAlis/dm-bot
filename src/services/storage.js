...
async function loadWorldContext(chatId) {
  ...
  return {
    town: { name: row.town_name, type: row.town_type, population: row.town_population, event: row.town_event, landmark: row.town_landmark },
    tavern: {
      name: row.tavern_name,
      wealth: row.tavern_wealth,
      feature: row.tavern_feature,
      rumor: row.tavern_rumor,
      specialBrew: { name: row.tavern_brew_name, desc: row.tavern_brew_desc },
    },
    npc: { summary: row.npc_summary, pocket: row.npc_pocket, secret: row.npc_secret },
...
async function loadGame(chatId) {
  ...
  const memoryResult = await pool.query('SELECT * FROM world_memory WHERE chat_id = $1 ORDER BY created_at DESC LIMIT 30', [chatId])
  const worldContext = await loadWorldContext(chatId)

  return {
    phase: gameRow.phase,
    numPlayers: gameRow.num_players,
    setupStep: gameRow.setup_step,
    setupSubStep: gameRow.setup_substep,
...
async function getChronicleEntries(chatId) {
  const result = await pool.query('SELECT entry, created_at FROM chronicle WHERE chat_id = $1 ORDER BY created_at ASC', [chatId])
  return result.rows
}
...
async function getActiveVote(chatId) {
  const result = await pool.query('SELECT * FROM votes WHERE chat_id = $1 ORDER BY created_at DESC LIMIT 1', [chatId])
